...
# [Include all the function definitions from the original code here]
...
def main():
    ...
    if st.sidebar.button("Train Model"):
        model, metrics, df = train_model_with_progress()
    ...
    if 'model' not in st.session_state:
        st.info("Please train the model using the sidebar button to start making predictions.")
        return
        ...
        with st.spinner("Generating forecast..."):
            try:
                predictions = predict_future(
                    st.session_state['model'],  # Changed from model[0]
                    df,
                    selected_commodity,
                    selected_apmc,
                    months
